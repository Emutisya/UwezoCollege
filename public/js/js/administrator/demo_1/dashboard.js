(function ($) {
  ...
  $(function () {
    ...
    if ($('#total-revenue').length) {
      ...
      try {
        (async () => {
          const response = await ajaxRequest('GET', '/payment/c/12')
          if (response.data) {
            ...
            $('#line_switch_2').on('click', async function (params) {
              const response = await getLineChartData(3)
              lineChart.data.labels = response.labels
              lineChart.data.datasets[0].data = response.values
              lineChart.update()
            })

            $('#line_switch_3').on('click', async function (params) {
              const response = await getLineChartData(6)
              lineChart.data.labels = response.labels
              lineChart.data.datasets[0].data = response.values
              lineChart.update()
            })

            $('#line_switch_4').on('click',async  function (params) {
              const response = await getLineChartData(12)
              lineChart.data.labels = response.labels
              lineChart.data.datasets[0].data = response.values
              lineChart.update()
            })
        ...
        console.log(error.responseJSON.message || 'Something went wrong while getting chart data');
      }

    }

    async function getLineChartData(months) {
      try {
        const response = await ajaxRequest('GET', `/payment/c/${months}`)
        if (response.data) {
